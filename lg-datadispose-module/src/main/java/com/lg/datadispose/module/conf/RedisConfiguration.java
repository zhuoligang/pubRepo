...
//			return new JedisConnectionFactory(sentinelConfiguration);
	...
	@SuppressWarnings({ "rawtypes", "unchecked" })
	@Bean("exchangeRateRedisTemplate")
	public RedisTemplate<String, Object> exchangeRateRedisTemplate() {
		// 设置序列化
		Jackson2JsonRedisSerializer jackson2JsonRedisSerializer = new Jackson2JsonRedisSerializer(Object.class);
		ObjectMapper om = new ObjectMapper();
		om.setVisibility(PropertyAccessor.ALL, JsonAutoDetect.Visibility.ANY);
		...
		jackson2JsonRedisSerializer.setObjectMapper(om);
		// 配置redisTemplate
		JedisConnectionFactory jedisConnectionFactory = jedisConnectionFactory(database1);
		RedisTemplate<String, Object> redisTemplate = new RedisTemplate<String, Object>();
		redisTemplate.setConnectionFactory(jedisConnectionFactory);
		RedisSerializer<?> stringSerializer = new StringRedisSerializer();
		redisTemplate.setKeySerializer(stringSerializer); // key序列化
