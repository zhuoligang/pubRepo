...
/**
 * 
* @ClassName: WebMQCustomerConfiguration
 ...
 */
@Configuration
public class DefaultMQCustomerConfiguration {

	private final static Logger logger = LoggerFactory.getLogger(DefaultMQCustomerConfiguration.class);
			...
			public ConsumeConcurrentlyStatus consumeMessage(List<MessageExt> msgs,ConsumeConcurrentlyContext context) {
				try {
					for (MessageExt msg : msgs) {
						String tag = msg.getProperties().get(MessageConst.PROPERTY_TAGS);
						publisher.publishEvent(new DefaultMQCustomerEvent(this, consumerTopic, tag, msg));
					}
					return ConsumeConcurrentlyStatus.CONSUME_SUCCESS;
				} catch (Exception e) {
					logger.error("用户中心消息处理异常", e);
